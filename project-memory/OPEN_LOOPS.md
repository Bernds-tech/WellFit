...
- Risk: R3
- Gap: Supabase Security Advisor still reports `pg_net` installed in the `public` extension schema. More importantly, live staging ACLs now show a separate least-privilege drift: schema `net` currently grants USAGE to PUBLIC, `anon` and `authenticated`, while pg_net routines including `http_get`, `http_post` and `http_delete` carry PUBLIC EXECUTE. This contradicts migration `016_internal_pg_net`, which explicitly revoked those privileges.
- Scope boundary: no external API exploit path, outbound request execution or data exposure was demonstrated in this audit. The supervisor deliberately did not call `net.http_*`. Therefore the finding remains YELLOW/R3 rather than a claimed production incident. Staging itself remains `ACTIVE_HEALTHY`, Edge version 7 is unchanged and 13 relevant IDEENWERK citizen/review/privacy/cluster tables are at zero synthetic baseline.
- Likely mechanism requiring proof: the enabled Supabase-managed event trigger `issue_pg_net_access` invokes `extensions.grant_pg_net_access()` after matching pg_net DDL and its current function definition grants schema usage to anon/authenticated/service_role. Current pg_net is version `0.20.4`; the exact later DDL/update event that restored privileges after migration 016 is not yet established.
- Evidence: live Supabase ACL/catalog checks and fresh Security Advisor observed at `2026-09-20T20:10:23Z`; `CTR-WERK-SEC-PGNET-ACL-001`; supervisor receipt `WERK_SUPERVISOR_2026-09-20T201023Z.json`.
- Remediation reference: https://supabase.com/docs/guides/database/database-linter?lint=0014_extension_in_public
- Next: Builder should prepare a bounded, reversible hardening change that (a) restores the intended no-PUBLIC/no-anon/no-authenticated privilege boundary, (b) accounts for Supabase pg_net extension event-trigger behavior so privileges do not silently reappear, and (c) preserves any proven internal WERK dependency. Do not move/drop pg_net or execute external HTTP calls merely to silence the advisor. After the change, Supervisor must re-run schema/function ACL checks, relevant IDEENWERK backend tests and Security Advisor.
- Close when: live ACL checks prove the intended least-privilege boundary durably, required internal behavior still passes, and either the extension-in-public warning is cleared or a documented platform constraint leaves it as a separately accepted staging hardening limitation. Production/security acceptance remains blocked while the ACL contradiction is open.
