...
async function pollUntilCompetenceVisible(startData) {
  const deadline = Date.now() + timeoutMs;
  let current = startData;
  while (Date.now() < deadline) {
    if (current?.competence_precheck && current?.current_status === 'precheck') return current;
    ...
    assert(dead.rows[0]?.count === 0, 'dead job found while waiting for competence precheck', dead.rows[0]);
    await new Promise(resolve => setTimeout(resolve, intervalMs));
    const polled = await fetchStatus();
    assert(polled.response.status === 200, 'protected status polling failed', polled.data);
    current = polled.data;
  }
  throw Object.assign(new Error('competence precheck did not become visible before timeout'), { detail: current });
}
...
async function assertNoResidue(cleanedPublicId) {
  ...
  assert(Object.values(row).every(value => value === 0), 'synthetic competence HTTP E2E residue remains after cleanup', row);
  return row;
}
...
try {
  ...
  const finalStatus = await pollUntilCompetenceVisible(firstStatus.data);
  assert(finalStatus.current_status === 'precheck', 'competence test must reach precheck', finalStatus);
  assertCompetence(finalStatus.competence_precheck, 'protected status');
  assert(finalStatus?.review_path?.depth === 'STANDARD', 'neutral competence fixture must remain on STANDARD procedural review', finalStatus);
  ...
  assert(denied.response.status === 403 && denied.data?.code === 'STATUS_ACCESS_DENIED', 'wrong token must not reveal competence result', denied.data);
  ...
  assert(checks?.original_text === payload.text, 'competence precheck must preserve immutable original_text', checks);
  ...
  assert(checks?.dead_jobs === 0 && checks?.active_jobs === 0 && checks?.done_jobs > 0, 'worker job state invalid after competence E2E', checks);
