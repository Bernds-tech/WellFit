...
def replace_section(path: str, header: str, replacement: str) -> None:
    p = Path(path)
    text = p.read_text()
    pat = re.compile(rf'(?ms)^{re.escape(header)}\n.*?(?=^## |\Z)')
    matches = list(pat.finditer(text))
    if len(matches) != 1:
        raise RuntimeError(f'{path}: expected one {header}, found {len(matches)}')
    m = matches[0]
    p.write_text(text[:m.start()] + replacement.rstrip() + '\n\n' + text[m.end():].lstrip('\n'))


replace_section('project-memory/TASK_LEDGER.md', '## WERK-IMPACT-001', block(f'''
## WERK-IMPACT-001
- Date: 2026-09-21
...
replace_section('project-memory/STARTED_WORK.md', '## WERK-IMPACT-001', block(f'''
## WERK-IMPACT-001
...
replace_section('project-memory/WORK_LOCKS.md', '## LOCK-WERK-IMPACT-001', block(f'''
...
replace_section('project-memory/OPEN_LOOPS.md', '## WERK-LOOP-IMPACT-001', block(f'''
...
replace_section('project-memory/DEPENDENCIES.md', '## WERK-DEP-IMPACT-FEEDBACK-001', block(f'''
...
tail = old.split(marker, 1)[1] if marker in old else ''
top = block(f'''
...
''')
if tail:
    top = top.rstrip() + marker + tail
nba.write_text(top.rstrip() + '\n')
...
Path(CLAIM_PATH).write_text(block(f'''
