...
if (!(p.taxes_and_social_contributions_mrd > s.taxes_plus_social_contributions[3])) fail('S.13+S.212 tax-page total must remain above S.13-only anchor in current data');
...
if (!String(p.reconciliation_rule).includes('nicht erzwungen gleichgesetzt')) fail('S.13/S.212 non-forced reconciliation guard missing');
...
if (!(selected2025 < s.tax_revenue[3])) fail('Selected 2025 tax items must remain an incomplete subset of total S.13 taxes');
...
if (Math.abs(u.wage_tax / 1000 - items.wage_tax[2]) < 0.05) fail('UG16 wage tax and S.13 wage tax must not be represented as identical values');
if (Math.abs(u.corporate_income_tax / 1000 - items.corporate_income_tax[2]) < 0.05) fail('UG16 KÖSt and S.13 KÖSt must not be represented as identical values');
...
if (!x.quantified_indirect_subsidies.every((val, i, a) => i === 0 || val > a[i - 1])) fail('Quantified tax-expenditure history expected increasing 2022-2024');
...
if (e.status !== 'selected-high-volume-measures-verified-not-full-93-ranking') fail('TAX-I status must explicitly reject full top-10 claim');
...
for (const m of e.measures) {
  if (!m.id || ids.has(m.id)) fail(`Duplicate/missing TAX-I measure id ${m.id}`);
  ids.add(m.id);
...
for (const token of ['vollständig eintreibbares', 'WERK-Einsparung', 'nicht addieren', 'dauerhafte Strukturwirkung', 'Top-10-Rangliste']) if (!prohibited.includes(token)) fail(`TAX-I prohibited-shortcut guard missing ${token}`);
...
if (t.coverage?.TAX_H_social_contributions_by_branch_and_sector !== 'open') fail('Canonical TAX-H umbrella must remain open/partial until employer/employee/sector detail is complete');
...
console.log(`Tax contract OK: S.13 2025 taxes+social EUR ${s.taxes_plus_social_contributions[3].toFixed(1)}bn; branch reference EUR ${(b.branch_reference_sum_mio[1] / 1000).toFixed(3)}bn remains non-identical to ESVG EUR ${(b.esvg_s13_net_social_contributions_mio[1] / 1000).toFixed(1)}bn; 10 selected tax expenditures EUR ${(e.selected_measure_sum_mio / 1000).toFixed(3)}bn cover ${e.selected_share_of_quantified_aggregate_pct.toFixed(1)}% of the quantified 2024 aggregate without being treated as collectable revenue.`);
