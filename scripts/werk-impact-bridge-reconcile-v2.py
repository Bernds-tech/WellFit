...
# Document only behavior already proven live on WERK Staging.
p=Path('werk-data/ideenwerk-api-contract.json'); d=json.loads(p.read_text())
...
for ep in d['endpoints']:
    if ep['method']=='GET' and ep['path']=='/status/{public_id}':
        ...
        if 'impact_bridge_check enthält' not in ep['notes']: ep['notes'] += '; impact_bridge_check enthält candidate_mapping mit bestehenden reform_ids/data_contract_ids/gate_refs und Registry-/Source-Versionen, no_known_mapping ohne Vollständigkeitsbehauptung oder bei Versionsdrift revalidation_required ohne alte Mappings; keine neue Wirkungszahl oder politische Entscheidung'
    if ep['method']=='GET' and ep['path']=='/privacy/export/{public_id}':
        if 'impact_bridge_check' not in ep['response']: ep['response'].append('impact_bridge_check')
        if 'Impact-Bridge' not in ep['notes']: ep['notes'] += '; der aktuelle stale-safe Impact-Bridge-Bezug wird als Provenienzinformation exportiert, ohne neue Wirkungszahl oder politische Bewertung'
...
p=Path('werk-data/werk-system-graph.json'); g=json.loads(p.read_text()); g['updated']='2026-09-21'
for n in g['nodes']:
    if n.get('id')=='IDEENWERK-IMPACT': n.update({'state':'IN_PROGRESS','builder_state':'IMPLEMENTED_STAGING_AWAITING_COUNTERCHECK','implementation':'ideenwerk-backend/sql/037_ideenwerk_impact_bridge.sql','registry':'werk-data/ideenwerk-impact-bridge.json','staging_migration':'20260920223436 ideenwerk_impact_bridge'})
for e in g['edges']:
...
def upsert(path,heading,body,insert_after=None):
    p=Path(path); t=p.read_text(); pat=rf'(?ms)^## {re.escape(heading)}\n.*?(?=^## |\Z)'
    if re.search(pat,t):
        t=re.sub(pat,body.rstrip()+'\n\n',t,count=1)
    elif insert_after and insert_after in t:
        t=t.replace(insert_after,insert_after+body.rstrip()+'\n\n',1)
    else:
        t=t.rstrip()+'\n\n'+body.rstrip()+'\n'
    p.write_text(t)

upsert('project-memory/STARTED_WORK.md','WERK-IDEENWERK-IMPACT-BRIDGE-001','''## WERK-IDEENWERK-IMPACT-BRIDGE-001
...
- Next: independent WERK Supervisor countercheck. Do not rebuild or advance the finishline gate.''','## Active work\n\n')

upsert('project-memory/WORK_LOCKS.md','LOCK-WERK-IDEENWERK-IMPACT-BRIDGE-001','''## LOCK-WERK-IDEENWERK-IMPACT-BRIDGE-001
...
- Release: only after independent Supervisor countercheck or a concrete contradiction.''','## Active/stale locks\n\n')

upsert('project-memory/TASK_LEDGER.md','WERK-IDEENWERK-IMPACT-BRIDGE-001','''## WERK-IDEENWERK-IMPACT-BRIDGE-001
- Date: 2026-09-20
- Status: IMPLEMENTED_NOT_VERIFIED
- Risk: R3
...
upsert('project-memory/OPEN_LOOPS.md','WERK-LOOP-IMPACT-BRIDGE-001','''## WERK-LOOP-IMPACT-BRIDGE-001
...
upsert('project-memory/DEPENDENCIES.md','WERK-DEP-IDEENWERK-IMPACT-001','''## WERK-DEP-IDEENWERK-IMPACT-001
