...
const reformCoverage=reforms.map(r=>({id:r.id,title:r.title,cost_description:r.costs,
  scenario_module:r.id==='SV-01'?'SV-01 (während Schuldentilgung)':modules.find(m=>m.id.split('/').includes(r.id))?.id??null,
