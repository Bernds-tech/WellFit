...
function resultText(run){
  const r=run.result||{};
  if(run.type==='Node Unit Tests'||run.type==='Node Kern-/Security') return `${r.passed??0}/${r.tests??(Number(r.passed||0)+Number(r.failed||0))} bestanden · ${r.failed??0} fehlgeschlagen`;
...
async function load(){try{
  ...
  const r=await fetch('./werk-data/ideenwerk-quality.json?v=20260903-v20',{cache:'no-store'});
  ...
  ${measured.length?`<div class="ey" style="margin-bottom:8px">Tatsächlich ausgeführte Tests</div><div class="iwQualityTests">${measured.map(t=>`<article class="iwQTest"><span class="iwQStatus">ausgeführt${t.executed_at?' · '+esc(t.executed_at):''}</span><h3>${esc(t.type)}</h3><p><b>${esc(resultText(t))}</b></p>${t.scope?`<p>${esc(t.scope)}</p>`:''}${t.environment||t.limitation?`<small>${t.environment?`<b>Umgebung:</b> ${esc(t.environment)}<br>`:''}${t.limitation?`<b>Einschränkung:</b> ${esc(t.limitation)}`:''}</small>`:''}</article>`).join('')}</div><div style="height:18px"></div>`:''}
